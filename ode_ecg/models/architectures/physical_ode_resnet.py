...
class ResGenerator(nn.Module):
    def __init__(self, device, physical_params):
        super(ResGenerator, self).__init__()
        self.z = torch.tensor(np.random.normal(0, 1, (40, 88))).float().to(device)
        self.device = device
        # [b, 1, 100]
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, t, x):
        if t == 1:
            ...
            self.z = torch.tensor(np.random.normal(0, 1, (x.shape[0], 88))).float().to(self.device)
        z = self.z
        t = t.repeat(40, 1)
        t = t.view(-1, 1)
        x = torch.cat([z, x, t, self.physical_params], dim=1)
        x = x.view(-1, 1, 100)
        x = self.conv_1(x)
        x = self.relu(x)
        x = self.residuals(x)
